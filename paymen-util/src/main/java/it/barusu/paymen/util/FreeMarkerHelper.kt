...
class FreeMarkerHelper(templatePaths: Array<String>) {
    ...
    var configuration = Configuration(Configuration.DEFAULT_INCOMPATIBLE_IMPROVEMENTS)
    ...
    fun render(template: Template, data: Map<String, Any>): String {
        try {
            ...
            log.error("Failed to render the template - ${template.name}, data: $data", ex)
            throw ApiException(msg = "Error rendering the template - ${template.name}", throwable = ex)
        }
